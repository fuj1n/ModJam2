...
public class GuiPassField extends Gui {
	/**
	...
	public GuiPassField(FontRenderer par1FontRenderer, int par2, int par3, int par4, int par5) {
	...
	public void drawTextBox() {
		if (this.getVisible()) {
			...
			String asterix = this.text.replaceAll("(?s).", "*");
			String s = this.fontRenderer.trimStringToWidth(asterix.substring(this.lineScrollOffset), this.getWidth());
			...
			boolean flag2 = this.cursorPosition < asterix.length() || asterix.length() >= this.getMaxStringLength();
			int k1 = j1;
